...
if (! class_exists('contentstudio')) {
    ...
    class ContentStudio
    {
        ...
        public function add_cstu_settings()
        {
            if (isset($_POST['data'])) {
                ...
                if (isset($_POST['data']['cs_save_in_wp'])) {
                    ...
                    if (add_option('contentstudio_save_media_in_wp', $cs_save_in_wp) == false) {
                    ...
                    echo json_encode(['status' => false, 'message' => 'Please select an option.']);
                    die();
                }
            } else {
            ...
            if (isset($_REQUEST) && isset($_REQUEST['cstu_create_new_post'], $_REQUEST['token'], $_REQUEST['nonce'])) {
                ...
                if ($valid) {
                    ...
                    if(get_option('contentstudio_save_media_in_wp', false)) {
                        // upload post images if the setting is enabled.
                        $post_response['upload_images_response'] = $this->upload_post_images($get_post->ID);
                    }
        ...
        /**
         * Upload post images
         * @param $post_id mixed The post id
         * @return array
         */
        private function upload_post_images($post_id)
        {
            try {
        ...
        private function uploadFeatureImages(&$response,$image,$post,$post_title) {
            try {
                ...
                if (isset($image) && $image) {
                    $image_data = wp_remote_get($image);
                    if (is_wp_error($image_data)) {
                        $response['status'] = false;
                        $response['warning_message'] = "Unable to download the post featured image.";
                    }
                    $status_code = $image_data['response']['code'] ?? 0;
